...
if __name__ == "__main__":
    ...
    xgb_model = xgb.XGBClassifier(
        learning_rate=0.01, n_estimators=200, verbose=1,
    )
    if args.tune == 1:
    ...
    acc_train, auc_train, nll_train, mse_train = compute_metrics(
        y_pred_train, y_train
    )
    acc_test, auc_test, nll_test, mse_test = compute_metrics(
        y_pred_test, y_test
    )
    print(
