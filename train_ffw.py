...
def train_ffw(
    train, val, model, optimizer, logger, saver, num_epochs, batch_size
):
    ...
    for epoch in range(num_epochs):
        ...
        for k in range(0, len(train_idxs), batch_size):
            ...
            train_auc = roc_auc_score(
                labels, torch.sigmoid(preds).detach().cpu()
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train feedforward neural network on sparse feature matrix"
    )
    ...
    train_df = pd.read_csv(
        f"data/{args.dataset}/preprocessed_data_train.csv", sep="\t"
    )
    test_df = pd.read_csv(
        f"data/{args.dataset}/preprocessed_data_test.csv", sep="\t"
    )
    ...
    model = FeedForward(
        train.shape[1] - 5, args.hid_size, args.drop_prob
    ).cuda()
    ...
    for k in range(0, test.shape[0], args.batch_size):
        ...
        inputs = inputs.cuda()
        with torch.no_grad():
            pred_test[k: k + args.batch_size] = (
                torch.sigmoid(model(inputs)).flatten().cpu().numpy()
            )
