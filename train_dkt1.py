    ...
    for epoch in range(num_epochs):
        ...
        for (
            ...
            for i in range(0, length, bptt):
                item_inp = (
                    item_inputs[:, i : i + bptt]
                    if item_inputs is not None
                    else None
                )
                skill_inp = (
                    skill_inputs[:, i : i + bptt]
            ...
            loss = compute_loss(
                preds, item_ids, skill_ids, labels.cuda(), criterion
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--item_in", action="store_true", help="If True, use items as inputs."
    )
    parser.add_argument(
        "--skill_in",
        action="store_true",
    ...
    assert (
        args.item_in or args.skill_in
    )  # Use at least one of skills or items as input
    assert (
