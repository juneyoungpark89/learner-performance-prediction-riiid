...
def df_to_sparse(df, Q_mat, active_features):
    ...
    Output:
        sparse_df (sparse array): sparse dataset where 
        first 5 columns are the same as in df
    """
    ...
    for key in ["a", "w"]:
        if key in active_features:
            if "tw" in active_features:
                features[key] = sparse.csr_matrix(
                    np.empty((0, (num_skills + 2) * NUM_WINDOWS))
                )
            else:
                features[key] = sparse.csr_matrix(
                    np.empty((0, num_skills + 2))
                )
    ...
    def process_user(user_id):
        ...
        if "a" in active_features:
            ...
            if "tw" in active_features:
                attempts = np.zeros(
                    (num_items_user, (num_skills + 2) * NUM_WINDOWS)
                )
                ...
                for i, (item_id, ts) in enumerate(df_user[:, 1:3]):
                    ...
                    if "tc" in active_features:
                        counts = phi(
                            np.array(counters[user_id].get_counters(ts))
                        )
            ...
            # Counts
                ...
                if "ic" in active_features:
                    ...
                    tmp = np.vstack(
                        (
                            np.zeros(item_ids_onehot.shape[1]),
                            np.cumsum(item_ids_onehot, 0),
                        )
                    )[:-1]
                    attempts[:, -2] = phi(
                        tmp[np.arange(num_items_user), df_user[:, 1]]
                    )
        ...
        if "w" in active_features:
                ...
                for i, (item_id, ts, correct) in enumerate(df_user[:, 1:4]):
                    ...
                    if "sc" in active_features:
                        for skill_id in Q_mat_dict[item_id]:
                            counts = phi(
                                np.array(
                                    counters[
                                        user_id, skill_id, "skill", "correct"
                                    ].get_counters(ts)
                                )
                            )
                            wins[
                    ...
                    if "ic" in active_features:
                        counts = phi(
                            np.array(
                                counters[
                                    user_id, item_id, "item", "correct"
                                ].get_counters(ts)
                            )
                        )
                        wins[i, -2 * NUM_WINDOWS: -1 * NUM_WINDOWS] = counts
                        if correct:
                            counters[user_id, item_id, "item", "correct"].push(
                                ts
                            )
                    ...
                    if "tc" in active_features:
                        counts = phi(
                            np.array(
                                counters[user_id, "correct"].get_counters(ts)
                            )
                        )
            ...
            # Counts
                ...
                if "sc" in active_features:
                    tmp = np.vstack((np.zeros(num_skills), skills))[:-1]
                    corrects = np.hstack((np.array(0), df_user[:, 3])).reshape(
                        -1, 1
                    )[:-1]
                    wins[:, :num_skills] = phi(
                        np.cumsum(tmp * corrects, 0) * skills
                    )
                ...
                if "ic" in active_features:
                    ...
                    tmp = np.vstack(
                        (
                            np.zeros(item_ids_onehot.shape[1]),
                            np.cumsum(item_ids_onehot * labels, 0),
                        )
                    )[:-1]
                    wins[:, -2] = phi(
                        tmp[np.arange(num_items_user), df_user[:, 1]]
                ...
                if "tc" in active_features:
                    wins[:, -1] = phi(
                        np.concatenate(
                            (np.zeros(1), np.cumsum(df_user[:, 3])[:-1])
                        )
                    )
    ...
    process_output = Parallel(n_jobs=-1, verbose=3)(
        map(delayed(process_user), user_list)
    )
    end_time = time.time()
    ...
    for single_output in tqdm(process_output):
        for key in single_output.keys():
            if key == "df":
                ...
                features[key] = sparse.vstack(
                    [features[key], single_output[key]]
                )
    end_time = time.time()
    ...
    if "u" in active_features:
        features["u"] = onehot.fit_transform(
            features["df"][:, 0].reshape(-1, 1)
        )
    if "i" in active_features:
        features["i"] = onehot.fit_transform(
            features["df"][:, 1].reshape(-1, 1)
        )

    X = sparse.hstack(
        [
            sparse.csr_matrix(features["df"]),
            sparse.hstack([features[x] for x in features.keys() if x != "df"]),
        ]
    ).tocsr()
    return X
...
if __name__ == "__main__":
    ...
    df = pd.read_csv(
        os.path.join(data_path, "preprocessed_data.csv"), sep="\t"
    )
