...
class DKT1(nn.Module):
    def __init__(
        ...
        self.lstm = nn.LSTM(
            self.input_size, hid_size, num_hid_layers, batch_first=True
        )
    ...
    def forward(self, item_inputs, skill_inputs, hidden=None):
        ...
        if (item_inputs is not None) and (skill_inputs is not None):
            item_onehots = F.one_hot(
                item_inputs, 2 * self.num_items + 1
            ).float()
            skill_onehots = F.one_hot(
                skill_inputs, 2 * self.num_skills + 1
            ).float()
